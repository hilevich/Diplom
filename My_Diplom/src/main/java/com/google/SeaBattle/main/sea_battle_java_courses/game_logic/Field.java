...
public class Field {
	...
	public static final int ALL_SHIPS_CELLS_COUNT = BOATS_COUNT * Ship.PATROL_BOAT_SIZE
			+ CRUISERS_COUNT * Ship.CRUISER_SIZE + DESTROYERS_COUNT * Ship.DESTROYER_SIZE
	...
	/**
	 * Проверка пространства на наличие кораблей x,y - координаты первой палубы
	 * rotation - поворот корабля (располагает корабль по горизонтали или по
	 * вертикали) boatSize - размер корабля (количество палуб)
	 */
	private boolean shipInPlace(int x, int y, boolean rotation, int boatSize) {
		...
		// положения корабля всего 2, поэтому проверяем одно условие, по else
		// присваиваются
		// другие значения
		if (rotation) {
			i = boatSize;
	...
	/**
	 * Расчет координат палуб x1,y1 - координаты первой палубы rotation -
	 * поворот корабля (располагается корабль по горизонтали или по вериткали)
	 */
