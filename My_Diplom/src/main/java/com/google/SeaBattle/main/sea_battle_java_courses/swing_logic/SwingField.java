...
import com.google.SeaBattle.main.sea_battle_java_courses.game_logic.Cell;
...
public class SwingField extends JComponent {
	...
	private void printLettersAndNumbers() {
		char[] letters = { 'À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Æ', 'Ç', 'È', 'Ê' };
		for (int i = 0; i < SECTOR_COUNT; i++) {
			...
			horizontal.setLocation(HEADER_ZONE + SPACE_BETWEEN * 2 + (Sector.WIDTH + SPACE_BETWEEN) * i, SPACE_BETWEEN);
			vertical.setLocation(SPACE_BETWEEN, HEADER_ZONE + SPACE_BETWEEN * 2 + (Sector.HEIGHT + SPACE_BETWEEN) * i);
			add(horizontal);
			add(vertical);
		}
	}
	...
	private void paintCurrentSelectedElement(Graphics2D g2d) {
		if (selectedSector != null) {
			...
			g2d.setPaint(new GradientPaint(spx + slw, SPACE_BETWEEN + HEADER_ZONE, Color.white, spx + slw / 4,
					SPACE_BETWEEN + HEADER_ZONE / 4, Color.green, true));
	...
	private void paintGradientBoxForFieldBottom(Graphics2D g) {
		...
		g.setPaint(new GradientPaint(lw, lh, Color.white, lw / 4, lh / 4, Color.lightGray, true));
