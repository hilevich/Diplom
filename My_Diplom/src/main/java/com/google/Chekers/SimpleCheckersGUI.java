...
public class SimpleCheckersGUI implements MouseListener, ActionListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		// The Piece's color should be equal to currentTurn, unless this is the
		// first move
		...
		/*
		 * Now, since we can be sure that the user has clicked on a piece of
		 * their own color, there are only four scenarios: 1) The user has
		 * nothing highlighted and wishes to highlight a new Square 2) The user
		 * wishes to change the Square that is selected 3) The user wishes to
		 * deselect the current Square 4) The user wishes to perform a jump
		 */

		if (sel.isOccupied() && selectedSquare == null) {
			// There is currently no square selected, so highlight all possible
			// moves
			selectedSquare = sel;
			...
			// The user has clicked on a different Piece than the one currently
			// selected
		...
		// Move-checking code
			...
			// The user is trying to make a move by moving from the
			// selectedSquare to the one they just clicked
			// First check to see if their choice corresponds to a square in
			// possibleMoves
			...
			for (Square choice : oldPossibleMoves) {
				if (choice.equals(sel)) {
					...
					// First, check to see if this was the first move being
					// performed
					if (currentTurn == Color.GREEN)
						currentTurn = selectedSquare.getOccupant().getColor();

					// Next, store in a variable whether or not a jump was
					// performed
					jumped = board.move(selectedSquare, sel);

					// Finally, indicate internally that a move has been found
					// and performed
					found = true;
	 ...
	 * 
	 * @param b
	 *            The Board to add to a JPanel
	 * @param p
	 *            the JPanel to be added to
	 */
	...
	public void updateStatus() {
		piecesLabel.setText(
				"Red pieces left: " + redCheckersLeft + "             Black pieces left: " + blackCheckersLeft);
	}

	/**
	 * Find out, if the game is over, who won and how that side won
	 * 
	 * @return A String containing the side which won, as well as how they won
	 *         (took other side's pieces, other side could make no more moves)
	 */
	public String winner() {
		...
		// Check second ending condition: one side cannot move its remaining
		// pieces
			...
			for (int j = 0; j < 8; j++) {
				...
				if (board.getSquare(i, j).isOccupied()) {
					...
					if (!potentialMoves.isEmpty()) {
						// The potentialMoves Vector contains at least one
						// square, so that side is capable of making a move
						// Find out what the color of the piece that can make
						// the move is, then set its <color>CanMove var to true
	...
	/**
	 * Instantiate this class to set the wheels of progress in beautiful
	 * event-driven motion
	 */
	public void chekers() {
