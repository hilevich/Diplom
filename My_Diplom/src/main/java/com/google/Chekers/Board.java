...
/**
 * Stores the game board as a 2D array of Squares. Also provides functionality
 * to move the Piece of one Square to another Square, and the ability to get all
 * possible moves of a particular Piece anywhere on the board
 * 
 * Note: The Board is the underlying structure that is drawn to the GUI in
 * CheckersGame.java
 *
...
public class Board {
	...
	/**
	 * Constructor takes no args and produces a Board of size rows x cols with
	 ...
	 * 
	 * @param row
	 *            The row at which the square should be
	 * @param col
	 *            The column at which the square should be
	 * 
	 ...
	 * 
	 * @param p
	 *            Piece for which moves should be found
	 * 
	 * @return A Vector of Squares to which this piece can move
	 */
	public Vector<Square> getPossibleMoves(Piece p) {
		/*
		 * Possible moves include up-left, up-right, down-left, down-right This
		 * corresponds to (row-- col--), (row-- col++), (row++ col--), (row++
		 * col++) respectively
		 */
		...
		// Begin checking which moves are possible, keeping in mind that only
		// black checkers may move up
	 ...
	 *
	 * @param p
	 *            The Piece whose possible moves we are highlighting
	 * @param doHighlight
	 *            Whether or not these possible moves should be highlighted
	 */
	...
	/**
	 * Perform a move on the board. This function does not perform input
	 * checking, as it is only called once a move has been validated by
	 * getPossibleMoves
	 * 
	 * 
	 * @param from
	 *            The square from which we are moving
	 * @param to
	 *            The square to which we are moving
	 * @return True if a jump has been performed, false if it's just a normal
	 *         move
	 */
	public boolean move(Square from, Square to) {
		...
		if (Math.abs(oldRow - newRow) > 1 || Math.abs(oldCol - newCol) > 1) {
			// A jump has been performed, so get the Square that lies between
			// from and to
